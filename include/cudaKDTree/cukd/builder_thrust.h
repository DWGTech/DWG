...
namespace cukd {
    ...
    */
    ...
    template<typename data_t,
        typename data_traits = default_data_traits<data_t>>
        void buildTree_DWG_thrust(/*! device-read/writeable array of data points */
            data_t* d_points,
            data_t* d_normals,
            /*! number of data points */
            int numPoints,
    ...
    namespace thrustSortBuilder {
        ...
        template<typename data_t, typename data_traits>
        struct ZipDWGCompare {
            explicit ZipDWGCompare(const int dim, const data_t* nodes)
                : dim(dim), nodes(nodes)
            {}
            ...
            /*! the actual comparison operator; will perform a
              ...
              sort key) */
            inline __both__ bool operator()
                (const thrust::tuple<uint32_t, data_t, data_t>& a,
                    const thrust::tuple<uint32_t, data_t, data_t>& b);
        ...
        */
        ...
        template<typename data_t, typename data_traits>
        inline __both__
            bool ZipDWGCompare<data_t, data_traits>::operator()
            (const thrust::tuple<uint32_t, data_t, data_t>& a,
                const thrust::tuple<uint32_t, data_t, data_t>& b)
        {
            using point_t = typename data_traits::point_t;
    ...
    template<typename data_t, typename data_traits>
    void buildTree_DWG_thrust(data_t* d_points,
        data_t* d_normals,
        int numPoints,
        box_t<typename data_traits::point_t>* worldBounds,
        cudaStream_t stream,
        ...
        GpuMemoryResource& memResource)
    {
        ...
        typedef thrust::tuple<tag_iterator, point_iterator, point_iterator> normal_iterator_tuple;
        typedef thrust::zip_iterator<normal_iterator_tuple> tag_point_normal_iterator;
        ...
        thrust::device_ptr<data_t> normals_begin(d_normals);
        thrust::device_ptr<data_t> normals_end(d_normals + numPoints);

        tag_point_normal_iterator begin = thrust::make_zip_iterator
        (thrust::make_tuple(tags.begin(), points_begin, normals_begin));
        tag_point_normal_iterator end = thrust::make_zip_iterator
        (thrust::make_tuple(tags.end(), points_end, normals_end));

        //data_t* tmp_d_points = 0;
        //cudaMallocManaged((char**)&tmp_d_points, numPoints * sizeof(*tmp_d_points));
        //thrust::device_ptr<data_t> tmp_points_begin(tmp_d_points);
        //thrust::device_ptr<data_t> tmp_points_end(tmp_d_points + numPoints);
        ...
        //thrust::device_vector<uint32_t> tmp_tags(numPoints);
        /* now build each level, one after another, cycling through the
           dimensoins */
        for (int level = 0; level < deepestLevel; level++) {
            //tmp_tags = tags;
            //cudaMemcpy(tmp_d_points, d_points, numPoints * sizeof(*d_points), cudaMemcpyDeviceToDevice);
            //tag_point_normal_iterator tag_normal_begin = thrust::make_zip_iterator
            //(thrust::make_tuple(tmp_tags.begin(), tmp_points_begin, normals_begin));
            //tag_point_normal_iterator tag_normal_end = thrust::make_zip_iterator
            //(thrust::make_tuple(tmp_tags.end(), tmp_points_end, normals_end));

            thrust::sort(thrust::device.on(stream), begin, end,
                ZipDWGCompare<data_t, data_traits>
                ((level) % num_dims, d_points));

            //thrust::sort(thrust::device.on(stream), tag_normal_begin, tag_normal_end,
            //    ZipDWGCompare<data_t, data_traits>
            //    ((level) % num_dims, d_points));
        ...
        //cudaFree(tmp_d_points);
        //tmp_d_points = nullptr;
    }
