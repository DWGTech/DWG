  ...
  namespace copyleft{
    namespace cgal{
                                        ...
                                        Eigen::PlainObjectBase<DerivedT> & T)
      {
        ...
        //std::cout << "begin point_area \n";
        
        A.setZero(n,1);
        T.setZero(n,3);

        //printf("P.(%d, %d), I.(%d, %d), N.(%d, %d)\n", P.rows(), P.cols(), I.rows(), I.cols(), N.rows(), N.cols());
        ...
        igl::parallel_for(P.rows(),[&](int i)
        {
          ...
          MatrixP neighbors;
          /*
          printf("i = %d,begin**************************************************, neighbor_index.rows() = %d, cols = %d \n",
              i, neighbor_index.rows(), neighbor_index.cols());
            */
          igl::slice(P,neighbor_index,(int)1,neighbors);

          //printf("i = %d, neighbors.rows = %d, cols = %d, here ******************************************************** here\n", i,neighbors.rows(), neighbors.cols());

          if(N.rows() && neighbors.rows() > 1){
              //printf("come in IFffffffffffffffffffffffffffffffffffffffffff \n");
          ...
          if(neighbors.rows() <= 2){
            ...
            /*
            printf("1.51.5.1.5&************************************************* score.rows = %d, cols = %d, special1.rows = %d, cols = %d, special2 = %d, %d\n\n\n", 
                scores.rows(), scores.cols(), igl::colon<int>(0, scores.rows() - 1).rows(), igl::colon<int>(0, scores.rows() - 1).cols(), 
                igl::colon<int>(0, 1).rows(), igl::colon<int>(0, 1).cols());
                */
            MatrixP plane;
            //printf("cnm slice !\n\n");
